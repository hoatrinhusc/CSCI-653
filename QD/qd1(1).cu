/*******************************************************************************
Quantum dynamics (QD) simulation of an electron in one dimension.

USAGE

%cc -o qd1 qd1.c -lm
%qd1 (input file qd1.in required; see qd1.h for the input format)
*******************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <mpi.h>
#include <cuda.h>
#include <omp.h>
#include "qd1.h"


void host2device(double *d1, double h2[NX+2][2], int offset)
{
        int i,j;
        size_t size = sizeof(double)*2*(nx+2);
        double *h1 = (double*)malloc(size);

        /* Store 2D arrays of host into 1D arrays */
        for (i=0; i<nx+2; i++) 
                for (j=0; j<2; j++) h1[2*i+j] = h2[offset+i][j];

        /* Copy array values from host to device */
        cudaMemcpy(d1, h1, size, cudaMemcpyHostToDevice);
	free(h1);
}

void device2host(double h2[NX+2][2], double *d1, int offset)
{
        int i,j;
        size_t size = sizeof(double)*2*(nx+2);
        double *h1 = (double*)malloc(size);

        /* Copy array values from host to device */
        cudaMemcpy(h1, d1, size, cudaMemcpyDeviceToHost);

        for (i=1; i<=nx; i++) {
                for (j=0; j<2; j++) {
                        h2[offset+i][j] = h1[2*i+j];
                }
        }
	free(h1);
}

/*----------------------------------------------------------------------------
        GPU calculation of Kinetic propagator
        1D array is used for communication between host and device.
        --> Modify the serial code to work with 1D array instead of 2D array.
------------------------------------------------------------------------------*/

__global__ void gpu_kin_prop(double *psi, double *wrk, double *al, double *blx, double *bux) {
        int tid = blockIdx.x*blockDim.x+threadIdx.x;
        int sx = tid+1;
        double wr, wi;

        /* WRK|PSI holds the new|old wave function */
                wr = al[0]*psi[2*sx+0]-al[1]*psi[2*sx+1];
                wi = al[0]*psi[2*sx+1]+al[1]*psi[2*sx+0];
                wr += (blx[2*sx+0]*psi[2*(sx-1)+0]-blx[2*sx+1]*psi[2*(sx-1)+1]);
                wi += (blx[2*sx+0]*psi[2*(sx-1)+1]+blx[2*sx+1]*psi[2*(sx-1)+0]);
                wr += (bux[2*sx+0]*psi[2*(sx+1)+0]-bux[2*sx+1]*psi[2*(sx+1)+1]);
                wi += (bux[2*sx+0]*psi[2*(sx+1)+1]+bux[2*sx+1]*psi[2*(sx+1)+0]);
                wrk[2*sx+0] = wr;
                wrk[2*sx+1] = wi;

}

__global__ void gpu_pot_prop(double *psi, double *u) {
        int tid = blockIdx.x*blockDim.x+threadIdx.x;
        int sx = tid+1;
        double wr, wi;

	wr=u[2*sx+0]*psi[2*sx+0]-u[2*sx+1]*psi[2*sx+1];
        wi=u[2*sx+0]*psi[2*sx+1]+u[2*sx+1]*psi[2*sx+0];
        psi[2*sx+0]=wr;
        psi[2*sx+1]=wi;
}

/*----------------------------------------------------------------------------*/
void pot_prop(double *dev_psi, double *dev_u, int offset) {
/*------------------------------------------------------------------------------
        Potential propagator for a half time step, DT/2.
------------------------------------------------------------------------------*/
        host2device(dev_psi, psi, offset);
        gpu_pot_prop <<<dimGrid, dimBlock>>> (dev_psi, dev_u);
        device2host(psi, dev_psi, offset);
        #pragma omp barrier                             /* Avoid race condtions */

}

void kin_prop(int t, int offset, double *dev_psi, double *dev_wrk, double *dev_al0, double *dev_al1, double *dev_blx0,
 double *dev_blx1, double *dev_bux0, double *dev_bux1) {
/*------------------------------------------------------------------------------
        Kinetic propagation for t (=0 for DT/2--half; 1 for DT--full) step.
-------------------------------------------------------------------------------*/

        /* Apply the periodic boundary condition */
        #pragma omp master
        periodic_bc();
        #pragma omp barrier

        host2device(dev_psi, psi, offset);
        if (t==0) {
                gpu_kin_prop <<<dimGrid, dimBlock>>> (dev_psi, dev_wrk, dev_al0, dev_blx0, dev_bux0);
        } else {
                gpu_kin_prop <<<dimGrid, dimBlock>>> (dev_psi, dev_wrk, dev_al1, dev_blx1, dev_bux1);}

        device2host(psi, dev_wrk, offset);
        #pragma omp barrier     
}

void single_step(int offset, double *dev_psi, double *dev_u, double *dev_wrk, double *dev_al0, double *dev_al1, double *dev_blx0, 
 double *dev_blx1, double *dev_bux0, double *dev_bux1) {
/*------------------------------------------------------------------------------
        Propagates the electron wave function for a unit time step, DT.
------------------------------------------------------------------------------*/
        pot_prop(dev_psi, dev_u, offset);  /* half step potential propagation */

        kin_prop(0,offset,dev_psi,dev_wrk,dev_al0,dev_al1,dev_blx0,dev_blx1,dev_bux0,dev_bux1); /* half step kinetic propagation   */
        kin_prop(1,offset,dev_psi,dev_wrk,dev_al0,dev_al1,dev_blx0,dev_blx1,dev_bux0,dev_bux1); /* full                            */
        kin_prop(0,offset,dev_psi,dev_wrk,dev_al0,dev_al1,dev_blx0,dev_blx1,dev_bux0,dev_bux1); /* half                            */

        pot_prop(dev_psi, dev_u, offset);  /* half step potential propagation */
}


int main(int argc, char **argv) {
	int step; 				/* Simulation loop iteration index */

	MPI_Init(&argc, &argv);
	MPI_Comm_rank(MPI_COMM_WORLD, &myid);
	MPI_Comm_size(MPI_COMM_WORLD, &nproc);
	omp_set_num_threads(NUM_DEVICE);
	nx=NX/NUM_DEVICE;
	init_param();  				/* Read input parameters */
	init_prop();   				/* Initialize the kinetic & potential propagators */
	init_wavefn(); 				/* Initialize the electron wave function */

	/* OMP hosts CUDA threads */
	#pragma omp parallel private(step)
	{
		int mpid, offset;
		int s, i, j;
		double bux0[NX+2][2], bux1[NX+2][2], blx0[NX+2][2], blx1[NX+2][2];
		double *dev_psi, *dev_wrk, *dev_u, *dev_blx0, *dev_blx1, *dev_bux0, *dev_bux1, *dev_al0, *dev_al1;
		double *host_al0, *host_al1;
		mpid = omp_get_thread_num(); 	/* Get OMP thread ID */
		offset = nx*mpid;
		cudaSetDevice(mpid%NUM_DEVICE);

		/*------------------------------------------------------------------------------------
        		Initializes wavefn, kin and pot propagators in the global memory of device.
			1D array stores both real and imaginary parts.
		-------------------------------------------------------------------------------------*/
		for (i=0; i<=NX+1; i++) {
			for (j=0; j<2; j++) bux0[i][j]= bux[0][i][j];
		}

		for (i=0; i<=NX+1; i++) {
                        for (j=0; j<2; j++) bux1[i][j]= bux[1][i][j];
                }

		for (i=0; i<=NX+1; i++) {
                        for (j=0; j<2; j++) blx0[i][j]= blx[0][i][j];
                }

                for (i=0; i<=NX+1; i++) {
                        for (j=0; j<2; j++) blx1[i][j]= blx[1][i][j];
			
                }

		size_t size1 = sizeof(double)*2*(nx+2);
		size_t size2 = sizeof(double)*2;
		cudaMalloc((void **) &dev_psi, size1);
		cudaMalloc((void **) &dev_wrk, size1);
		cudaMalloc((void **) &dev_u, size1);
		cudaMalloc((void **) &dev_blx0, size1);
		cudaMalloc((void **) &dev_blx1, size1);
		cudaMalloc((void **) &dev_bux0, size1);
		cudaMalloc((void **) &dev_bux1, size1);
		cudaMalloc((void **) &dev_al0, size2);
		cudaMalloc((void **) &dev_al1, size2);

		host2device(dev_psi, psi, offset);
		host2device(dev_wrk, wrk, offset);
		host2device(dev_u, u, offset);
		host2device(dev_blx0, blx0, offset);
		host2device(dev_blx1, blx1, offset);
		host2device(dev_bux0, bux0, offset);
		host2device(dev_bux1, bux1, offset);

		host_al0 = (double*)malloc(size2);
                host_al1 = (double*)malloc(size2);

		for (s=0; s<2; s++) host_al0[s] = al[0][s];
		for (s=0; s<2; s++) host_al1[s] = al[1][s];			

		/* Copy array values from host to device */
		cudaMemcpy(dev_al0, host_al0, size2, cudaMemcpyHostToDevice);
		cudaMemcpy(dev_al1, host_al1, size2, cudaMemcpyHostToDevice);
		free(host_al0);
		free(host_al1);

		for (step=1; step<=NSTEP; step++) {
			single_step(offset,dev_psi,dev_u,dev_wrk,dev_al0,dev_al1,dev_blx0,dev_blx1,dev_bux0,dev_bux1);

			if (step%NECAL==0) {
				#pragma omp master
					calc_energy();	// Let one thread does message passing
				#pragma omp barrrier 

				if ((myid==0)&&(mpid==0)) printf("%lf %lf %lf %lf\n",DT*step,eking,epotg,etot);
			}
		}
		
		cudaFree(dev_psi);
	        cudaFree(dev_wrk);
        	cudaFree(dev_al0);
        	cudaFree(dev_al1);
       		cudaFree(dev_blx0);
        	cudaFree(dev_blx1);
        	cudaFree(dev_bux0);
        	cudaFree(dev_bux1);
        	cudaFree(dev_u);

	}

	MPI_Finalize();

	return 0;
}

/*----------------------------------------------------------------------------*/
void init_param() {
/*------------------------------------------------------------------------------
	Initializes parameters by reading them from input file.
------------------------------------------------------------------------------*/
	FILE *fp;

	/* Read control parameters */
	fp = fopen("qd1.in","r");
	fscanf(fp,"%le",&LX);
	fscanf(fp,"%le",&DT);
	fscanf(fp,"%d",&NSTEP);
	fscanf(fp,"%d",&NECAL);
	fscanf(fp,"%le%le%le",&X0,&S0,&E0);
	fscanf(fp,"%le%le",&BH,&BW);
	fscanf(fp,"%le",&EH);
	fclose(fp);

	/* Calculate the mesh size */
	dx = LX/NX;
}

/*----------------------------------------------------------------------------*/
void init_prop() {
/*------------------------------------------------------------------------------
	Initializes the kinetic & potential propagators.
------------------------------------------------------------------------------*/
	int stp,s,i,up,lw;
	double a,exp_p[2],ep[2],em[2];
	double x;

	/* Set up kinetic propagators */
	a = 0.5/(dx*dx);

	for (stp=0; stp<2; stp++) { /* Loop over half & full steps */
		exp_p[0] = cos(-(stp+1)*DT*a);
		exp_p[1] = sin(-(stp+1)*DT*a);
		ep[0] = 0.5*(1.0+exp_p[0]);
		ep[1] = 0.5*exp_p[1];
		em[0] = 0.5*(1.0-exp_p[0]);
		em[1] = -0.5*exp_p[1];

		/* Diagonal propagator */
		for (s=0; s<2; s++) al[stp][s] = ep[s];

		/* Upper & lower subdiagonal propagators */
		for (i=1; i<=NX; i++) { /* Loop over mesh points */
			if (stp==0) { /* Half-step */
				up = i%2;     /* Odd mesh point has upper off-diagonal */
				lw = (i+1)%2; /* Even               lower              */
			}
			else { /* Full step */
				up = (i+1)%2; /* Even mesh point has upper off-diagonal */
				lw = i%2;     /* Odd                 lower              */
			}
			for (s=0; s<2; s++) {
				bux[stp][i][s] = up*em[s];
				blx[stp][i][s] = lw*em[s];
			}
		} /* Endfor mesh points, i */
	} /* Endfor half & full steps, stp */

	/* Set up potential propagator */
	for (i=1; i<=NX; i++) {
		x = dx*i+myid*LX;
		/* Construct the edge potential */
		if ((myid==0)&&(i==1) || (myid==nproc-1)&&(i==NX))
			v[i] = EH;
		/* Construct the barrier potential */
		else if (0.5*(LX*nproc-BW)<x && x<0.5*(LX*nproc+BW))
			v[i] = BH;
		else
			v[i] = 0.0;
		/* Half-step potential propagator */
		u[i][0] = cos(-0.5*DT*v[i]);
		u[i][1] = sin(-0.5*DT*v[i]);
	}
}

/*----------------------------------------------------------------------------*/
void init_wavefn() {
/*------------------------------------------------------------------------------
	Initializes the wave function as a traveling Gaussian wave packet.
------------------------------------------------------------------------------*/
	int sx,s;
	double x,gauss,psisq,norm_fac;
	double psisqg;

	/* Calculate the the wave function value mesh point-by-point */
	for (sx=1; sx<=NX; sx++) {
		x = dx*sx-X0+myid*LX;
		gauss = exp(-0.25*x*x/(S0*S0));
		psi[sx][0] = gauss*cos(sqrt(2.0*E0)*x);
		psi[sx][1] = gauss*sin(sqrt(2.0*E0)*x);
	}

	/* Normalize the wave function */
	psisq=0.0;
	for (sx=1; sx<=NX; sx++)
		for (s=0; s<2; s++)
			psisq += psi[sx][s]*psi[sx][s];
	psisq *= dx;
	MPI_Allreduce(&psisq,&psisqg,1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	norm_fac = 1.0/sqrt(psisqg);
	for (sx=1; sx<=NX; sx++)
		for (s=0; s<2; s++)
			psi[sx][s] *= norm_fac;
}


/*----------------------------------------------------------------------------*/
void periodic_bc() {
/*------------------------------------------------------------------------------
	Applies the periodic boundary condition to wave function PSI, by copying
	the boundary values to the auxiliary array positions at the other ends.
------------------------------------------------------------------------------*/
	int s;
	int plw, pup;
	double dbuf[2], dbufr[2];

	MPI_Request request[2];
	MPI_Status status[2];

	plw = (myid-1+nproc)%nproc;
	pup = (myid+1)%nproc;

	for (s=0; s<2; s++) dbuf[s]=psi[1][s];
	MPI_Irecv(&dbufr,2, MPI_DOUBLE, pup, 20, MPI_COMM_WORLD, &request[0]);
	MPI_Send(&dbuf,2, MPI_DOUBLE, plw, 20, MPI_COMM_WORLD);
	MPI_Wait(&request[0], &status[0]);

	/* Copy boundary wave function values */
	for (s=0; s<=1; s++) {
		psi[NX+1][s] = dbufr[s];
	}

	for (s=0; s<2; s++) dbuf[s]=psi[NX][s];
        MPI_Irecv(&dbufr,2, MPI_DOUBLE, plw, 10, MPI_COMM_WORLD, &request[1]);
        MPI_Send(&dbuf,2, MPI_DOUBLE, pup, 10, MPI_COMM_WORLD);
        MPI_Wait(&request[1], &status[1]);

        /* Copy boundary wave function values */
        for (s=0; s<=1; s++) {
                psi[0][s] = dbufr[s];
        }

}

/*----------------------------------------------------------------------------*/
void calc_energy() {
/*------------------------------------------------------------------------------
	Calculates the kinetic, potential & total energies, EKIN, EPOT & ETOT.
------------------------------------------------------------------------------*/
	int sx,s;
	double a,bx;

	/* Apply the periodic boundary condition */
//	#pragma omp master
        periodic_bc();
//        #pragma omp barrier

	/* Tridiagonal kinetic-energy operators */
	a =   1.0/(dx*dx);
	bx = -0.5/(dx*dx);

	/* |WRK> = (-1/2)Laplacian|PSI> */
	for (sx=1; sx<=NX; sx++)
		for (s=0; s<=1; s++)
			wrk[sx][s] = a*psi[sx][s]+bx*(psi[sx-1][s]+psi[sx+1][s]);

	/* Kinetic energy = <PSI|(-1/2)Laplacian|PSI> = <PSI|WRK> */
	ekin = 0.0;
	for (sx=1; sx<=NX; sx++)
		ekin += (psi[sx][0]*wrk[sx][0]+psi[sx][1]*wrk[sx][1]);
	ekin *= dx;

	MPI_Allreduce(&ekin, &eking, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

	/* Potential energy */
	epot = 0.0;
	for (sx=1; sx<=NX; sx++)
		epot += v[sx]*(psi[sx][0]*psi[sx][0]+psi[sx][1]*psi[sx][1]);
	epot *= dx;

	MPI_Allreduce(&epot, &epotg, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

	/* Total energy */
	etot = eking+epotg;
}

